...
it is natural to expect we will similarly estimate 
geographic maps of ``effective population density'',
perhaps equal to actual local population density scaled by $\E[X(X-1)]$ as above.
This has been done for homogeneous space (see \citep{barton_depaulis_etheridge}),
but precise notions for inhomogeneous geographies are elusive.
...
Leaving aside these caveats about overall scaling,
we expect to be able to at least estimate \emph{relative} population density across space.
How can we do this?
What signature does varying population density leave 
...
Said in the backwards-in-time way common to population genetics,
when looking for the parents of a given two individuals,
...
we expect samples from that valley to be identical (by descent)
on larger portions of their genomes,
...
Consider two individuals that are close enough to plausibly be siblings.
Loosely following the argument above,
the chance that they \emph{are} siblings is inversely proportional to
the number of individuals who could plausibly be their parents.
This number of ``possible parents'' is proportional to
Wright's neighborhood size, $N_\text{loc} = 4 \pi \rho \sigma_e^2$,
...
More distant relatives give increasingly vauge information
about population density in larger regions.
...
% \emph{Wright's neighborhood size}, % denoted $N_\text{loc}$,
% defined to be the average \emph{effective} number of individuals
...
Genealogical relationships form on a time scale given by population density,
because in a geographic region having $N$ individuals
the chance two individuals are siblings is of order $1/N$ --
and so
a given lineage has only a chance of order $1/N$ to coalesce with another given lineage in that region.
...
If the migration rate into a geographic region is also of rate $1/N$ --
i.e., if only a few individuals in each generation had parents living outside of the region --
then the pedigree in a region can be thought of more or less autonomously.
This is the parameter regime in which the structured coalescent applies.
In other words,
regions cannot be analyzed independently
unless a region of interest sees less than one migrant per generation,


Since genetic estimates of population size 
...
Similarly, lineages in the pedgiree from a geographic region that acts as a population ``sink'' 
tend to leave, and coalesce in the nearby ``source'' area,
and so rates of coalescence will depend more on the population size in the source area
than on the entire range.
