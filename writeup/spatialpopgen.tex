...
is the population density in a given region within that area.
What exactly do we mean by population density?
One definition is simply the total number of individuals in a given area.
However, such a count may include individuals who are not reproductive 
(e.g., juveniles), 
or who transient and not reproductive locally 
...
the \textit{effective} number of individuals per unit area in a given region.
This quantity is analogous to effective population size ($N_e$), 
but may vary across different different parts of a species' range, 
and can therefore not be summarize with a single number.
...
The effective population density we estimate using coalescence rates
is ``effective'' partly because of this difference.

So, what can the spatial pedigree tell us about local population density?
...
%        \includegraphics[width=\linewidth]{figs/pedigree.pdf}
    ...
    \end{subfigure}
        \caption{
		\todo{the effect of population density on tmrcas} \triage{heterozygosity}
		}
        \label{pop_density}
\end{figure}
...
\begin{figure}[ht]
    ...
    \begin{subfigure}{\textwidth}
        \centering
%        \includegraphics[width=\linewidth]{figs/spatial_pedigree.pdf}
        \caption{\todo{ibd map for 2 individuals, one close to barrier, one far away}}
        \label{ibd}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
        \centering
...
\todo{Relate to $N_e$-over-time things.}
...
\todo{ What proportion of ancestry of one side of the mountain comes from the other side of the mountain,
		as a function of time ago? \cite{knowlton1994}}
