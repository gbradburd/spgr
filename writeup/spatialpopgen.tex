...
%Abstract
\begin{abstract}
    Many questions that we have about the history and dynamics of current populations of organisms
    ...
    and where did the anscestors of an individual today live?
    Mundane answers to these questions can have profound consequences
    for our understanding of history, ecology, and the evolutionary process.
    % Unfortunately, the complexities of continuous, geographical space
    % has proven difficult to capture in theoretical or statistical models.
    In this review, we discuss ways that geographic aspects of
    population distribution, movement, and reproduction 
    are reflected in the population's history across space and time
    -- its' pedigree, embedded in geographical space,
    or ``spatial pedigree''.
    Our aim is to provide intuition for how these processes are reflected in genetic data,
    since the pedigree's structure is what determines 
    patterns of relatedness in modern genetic variation.
    We also summarize some current methods of geographical population genetics,
    and highlight current gaps in the statistical toolbox
...
methods that are based on theory
...
evolving across geographic landscapes.
We produced the examples used for illustration
with SLiM v3.1 \citep{haller2018forward},
...
and then used tools from \texttt{tskit} to extract relatedness information,
...
\begin{figure}	%[h]
    ...
    \end{subfigure}
        \caption{
            \textbf{(left)} Map of population density of the simulated population set-up
...
Perhaps the most important avenue for future work is to 
...
how locally adaptive alleles can be maintained \citep{slatkin_local,barton,ralph_patchy},
or how hybrid zones are maintained \citep{barton_hybrid,sedghifar}.
